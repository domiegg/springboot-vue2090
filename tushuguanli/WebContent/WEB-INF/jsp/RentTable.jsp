...
<tr bgcolor="#E7E7E7">
	<td height="24" colspan="10" background="image/image/tbg.gif">&nbsp;我要还书&nbsp;</td>
</tr>
...
<fmt:parseDate value="${rent['rentTime']}" pattern="yyyy-MM-dd" var="rentTime" />
	 <fmt:formatDate value="${rentTime }" var="rt" />
...
<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
	...
	<td>${rent['rentTime'] }</td>
	<td>${rent['limitTime'] }
								<c:set var="result" value="${nowDate-limitTime.getTime() }"></c:set>	
	 							<c:if test="${limitTime.getTime()-nowDate lt 0 }" var="Expired">
									<font color="red">您已超期,罚款:<fmt:formatNumber type="number" value="${result/1000/60/60/24 }" maxFractionDigits="2"/>元</font>
									</c:if>
	</td>
	<td><a href="sys/rent/update?id=${rent['id'] }" onclick="return check();">还书</a></td>
</tr>
</c:forEach>
...
<tr align="right" bgcolor="#EEF4EA">
	...
	<c:if test="${pager.pageNow gt 1}">
                <a href="sys/rent/query?pageNow=${pager.pageNow-1 }">上一页</a>
                </c:if>
                <c:if test="${pager.pageNow lt pager.totalPage}">
                <a href="sys/rent/query?pageNow=${pager.pageNow+1 }">下一页</a>
                </c:if></td>
</tr>
</table>
