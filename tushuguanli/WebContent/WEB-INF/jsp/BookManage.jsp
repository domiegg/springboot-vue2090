...
<tr bgcolor="#E7E7E7">
	<td height="24" colspan="10" background="image/image/tbg.gif">&nbsp;图书管理&nbsp;</td>
</tr>
...
<c:forEach items="${pager.datas }" var="book" varStatus="vs">
<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
	<td>${book['bookName'] }</td>
	<td>${book['bookType'] }</td>
	<td>${book['bookWriter'] }</td>
	<td>${book['bookHouse'] }</td>
	<td>${book['bookNumber'] }</td>
	<td><a href="sys/book/Edit?id=${book['id'] }">编辑</a> | <a href="sys/book/delete?id=${book['id'] }" onclick="return DCheck();">报废</a></td>
</tr>
</c:forEach>
...
<!--  搜索表单  -->
<form name='' action='sys/book/manage' method='post'>
    ...
    <td background='image/image/wbg.gif' align='center'>
      <table border='0' cellpadding='0' cellspacing='0'>
        ...
        <td>
          ...
          <input name="pageNow" type="hidden" id="pageNow" value="${pager.pageNow }">
        </td>
        <td>
        	<c:if test="${pager.pageNow gt 1}">
                <a href="sys/book/manage?pageNow=${pager.pageNow-1 }">上一页</a>
                </c:if>
                <c:if test="${pager.pageNow lt pager.totalPage}">
                <a href="sys/book/manage?pageNow=${pager.pageNow+1 }">下一页</a>
                </c:if>
        </td>
       </tr>
      </table>
