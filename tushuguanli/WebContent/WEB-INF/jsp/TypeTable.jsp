...
<tr bgcolor="#E7E7E7">
	<td height="24" colspan="10" background="image/image/tbg.gif">&nbsp;类别管理&nbsp;<a href="sys/request/typeAdd" style="color: blue">添加</a></td>
</tr>
...
<c:forEach items="${pager.datas }" var="type" varStatus="vs">
	<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
		...
		<td><c:out value="图书"></c:out></td>
		</c:if>
		<c:if test="${type['status'] eq 2}">
		<td><c:out value="期刊"></c:out></td>
		</c:if>
		<c:if test="${type['status'] eq '' or type['status'] eq null}">
		<td><c:out value="暂无"></c:out></td>
		</c:if>
		<td><a href="sys/type/queryOne?id=${type['id']}">查看</a> | <a href="sys/type/typeDelete?id=${type['id']}" onclick="return check();">报废</a></td>
	</tr>
</c:forEach>
...
<tr align="right" bgcolor="#EEF4EA">
	...
	<c:if test="${pager.pageNow gt 1}">
                <a href="sys/type/query?pageNow=${pager.pageNow-1 }">上一页</a>
                </c:if>
                <c:if test="${pager.pageNow lt pager.totalPage}">
                <a href="sys/type/query?pageNow=${pager.pageNow+1 }">下一页</a>
                </c:if>
	</td>
</tr>
</table>
