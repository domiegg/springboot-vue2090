   ...
   <input type="hidden" name="id" value="${type['id'] }"/>
	   <table class="table table-bordered" >
	     <tr>
	       ...
	       <td colspan="3"><input type="text" name="typeName" id="typeName" value="${type['typeName'] }"/></td>
	     </tr>
	      <tr>
	       <td nowrap="nowrap">类型类别：</td>
	       <td colspan="3">
		       <select name="status" id="status">
		       			<c:if test="${type['status'] eq '1' }" var="s1">
		       				<option value="1" selected="selected">图书</option>
		       			</c:if>
		       			<c:if test="${!s1}">
		       				<option value="1">图书</option>
		       			</c:if>
		       			<c:if test="${type['status'] eq '2' }" var="s2">
		       				<option value="2" selected="selected">期刊</option>
		       			</c:if>
		       			<c:if test="${!s2}">
